...
def ck_q_learning(
    ...
    return QLearningBrain(
        ...
        state_encoder=_ck_state_encoder,
...
def _ck_payoff_func(my_action: Action, opp_action: Action) -> float:
    matrix = {
        (Action.COOPERATE, Action.COOPERATE): 3.0,
        (Action.COOPERATE, Action.DEFECT): 1.0,
        (Action.DEFECT, Action.COOPERATE): 5.0,
        (Action.DEFECT, Action.DEFECT): -5.0,
    }
    return matrix[(my_action, opp_action)]


def ck_best_response() -> BestResponseBrain:
    return BestResponseBrain(
        action_space=[Action.COOPERATE, Action.DEFECT],
        payoff_func=_ck_payoff_func,
    )
