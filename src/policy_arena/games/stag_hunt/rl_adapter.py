...
def _sh_state_encoder(obs) -> str:
    """State = opponent's last action (or 'start' if first round)."""
...
def sh_q_learning(
    ...
    return QLearningBrain(
        ...
        state_encoder=_sh_state_encoder,
...
def _sh_payoff_func(my_action: Action, opp_action: Action) -> float:
    """Standard Stag Hunt payoffs."""
    matrix = {
        (Action.COOPERATE, Action.COOPERATE): 4.0,
        (Action.COOPERATE, Action.DEFECT): 0.0,
        (Action.DEFECT, Action.COOPERATE): 3.0,
        (Action.DEFECT, Action.DEFECT): 2.0,
    }
    return matrix[(my_action, opp_action)]


def sh_best_response() -> BestResponseBrain:
    return BestResponseBrain(
        action_space=[Action.COOPERATE, Action.DEFECT],
        payoff_func=_sh_payoff_func,
    )
