"""Iterated Game of Chicken with round-robin pairwise matching.

Two drivers race toward each other. Each independently chooses to Swerve or
go Straight:
  - (Swerve, Swerve)     = (3, 3)   — both safe, tie
  - (Swerve, Straight)   = (1, 5)   — you're chicken, they win
  - (Straight, Swerve)   = (5, 1)   — you win, they're chicken
  - (Straight, Straight)  = (-5, -5) — crash! catastrophic

The key tension: going Straight dominates if the other Swerves, but mutual
Straight is the worst outcome for both. This creates a game of nerve.

COOPERATE = Swerve, DEFECT = Straight.
"""
...
DEFAULT_PAYOFF_MATRIX: dict[tuple[Action, Action], tuple[float, float]] = {
    (Action.COOPERATE, Action.COOPERATE): (3.0, 3.0),  # Both swerve — safe, tie
    (Action.COOPERATE, Action.DEFECT): (1.0, 5.0),  # You swerve, they go straight
    (Action.DEFECT, Action.COOPERATE): (5.0, 1.0),  # You go straight, they swerve
    (Action.DEFECT, Action.DEFECT): (-5.0, -5.0),  # Both straight — crash!
}


class ChickenModel(mesa.Model):
    """Iterated Game of Chicken with round-robin pairwise matching."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Max welfare = both swerve (3+3=6) per pair
        cc = self.payoff_matrix[(Action.COOPERATE, Action.COOPERATE)]
        cd = self.payoff_matrix[(Action.COOPERATE, Action.DEFECT)]
        dc = self.payoff_matrix[(Action.DEFECT, Action.COOPERATE)]
        dd = self.payoff_matrix[(Action.DEFECT, Action.DEFECT)]
        self._max_welfare_per_pair: float = max(
            cc[0] + cc[1], cd[0] + cd[1], dc[0] + dc[1], dd[0] + dd[1]
        )
        self._round_max_payoff: float = 0.0

        for i, brain in enumerate(brains):
            label = labels[i] if labels else None
            ChickenAgent(self, brain=brain, label=label)

        self.datacollector = mesa.DataCollector(
            model_reporters={
                "cooperation_rate": lambda m: self._metric_swerve_rate(),
    ...
    def _metric_swerve_rate(self) -> float:
        """Fraction of agents choosing Swerve (COOPERATE)."""
    ...
    def _metric_nash_distance(self) -> float:
        ...
        Chicken has TWO pure NE: (Swerve, Straight) and (Straight, Swerve).
        Both have a swerve rate of 0.5 across the pair. We also consider
        the extremes all-swerve and all-straight as reference points.
        ...
        a_count = sum(1 for a in self._round_all_actions if a == Action.COOPERATE)
        swerve_rate = a_count / n
        # Pure NE are at swerve_rate = 1.0 (all swerve) and 0.0 (all straight)
        # But Chicken's actual pure NE are asymmetric pairs at swerve_rate ~ 0.5
        dist_all_swerve = abs(swerve_rate - 1.0)
        dist_all_straight = abs(swerve_rate - 0.0)
        return min(dist_all_swerve, dist_all_straight)
