"""Hawk-Dove agent — delegates decision-making to a Brain."""
...
class HDAgent(mesa.Agent):
    """Agent in an iterated Hawk-Dove game.

    Uses COOPERATE = Dove, DEFECT = Hawk.
    Two players compete over a resource. Doves share, hawks fight.
    If both are doves, they share the resource equally.
    If a hawk meets a dove, the hawk takes all.
    If two hawks meet, both get injured.
    """
    ...
    def __init__(self, model: mesa.Model, brain: Brain, label: str | None = None):
        ...
        self.cooperated: float = 1.0  # Dove rate for the round
    ...
    def end_round(self) -> None:
        if self._round_actions:
            a_count = sum(1 for a in self._round_actions if a == Action.COOPERATE)
            self.cooperated = a_count / len(self._round_actions)
        else:
            self.cooperated = 1.0
        ...
        if self._round_opponent_results:
            ...
            for idx, (_opp_id, (my_act, opp_act, payoff)) in enumerate(
                ...
                my_str = "D" if my_act == Action.COOPERATE else "H"
                opp_str = "D" if opp_act == Action.COOPERATE else "H"
                parts.append(
