"""Battle of the Sexes agent — delegates decision-making to a Brain."""
...
class BoSAgent(mesa.Agent):
    """Agent in an iterated Battle of the Sexes.

    Uses COOPERATE = Option A, DEFECT = Option B.
    The payoff matrix is asymmetric: row player prefers A, column player prefers B.
    In round-robin, each agent plays both sides across different pairings.
    """
    ...
    def __init__(self, model: mesa.Model, brain: Brain, label: str | None = None):
        ...
        self.cooperated: float = 1.0  # Option A rate for the round
    ...
    def end_round(self) -> None:
        ...
        if self._round_opponent_results:
            ...
            for idx, (_opp_id, (my_act, opp_act, payoff)) in enumerate(
                ...
                my_str = "A" if my_act == Action.COOPERATE else "B"
                opp_str = "A" if opp_act == Action.COOPERATE else "B"
                parts.append(
