"""Chicken game agent — delegates decision-making to a Brain."""
...
class ChickenAgent(mesa.Agent):
    """Agent in an iterated Chicken game.

    Uses COOPERATE = Swerve, DEFECT = Straight.
    Two drivers race toward each other — swerving is safe but "chicken,"
    going straight wins if the other swerves, but mutual straight is a crash.
    """
    ...
    def __init__(self, model: mesa.Model, brain: Brain, label: str | None = None):
        ...
        self.cooperated: float = 1.0  # Swerve rate for the round
    ...
    def end_round(self) -> None:
        ...
        if self._round_opponent_results:
            ...
            for idx, (_opp_id, (my_act, opp_act, payoff)) in enumerate(
                ...
                my_str = "Sw" if my_act == Action.COOPERATE else "St"
                opp_str = "Sw" if opp_act == Action.COOPERATE else "St"
                parts.append(
